...
void setUp(void) {
  // Save the original value of the environment variable if it exists
    original_prompt = getenv("MY_PROMPT") ? strdup(getenv("MY_PROMPT")) : NULL;

    // Set a default value for MY_PROMPT for the tests
    setenv("MY_PROMPT", "shell>", 1);
    setenv("HOME", "/home/codespace", 1);
}

void tearDown(void) {
  if (original_prompt) {
        setenv("MY_PROMPT", original_prompt, 1);
        free(original_prompt); // Free the saved value
    } else {
        unsetenv("MY_PROMPT"); // Remove the environment variable if it didn't exist before
    }
}
